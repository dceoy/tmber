...
def create_bed_from_fa(fa_path, dest_dir_path, bgzip='bgzip',
    ...
    print_log('Identify target cases: {}'.format(set(target_cases)))
    with ProcessPoolExecutor(max_workers=n_cpu) as x:
        fs = [
            x.submit(_identify_target_region, k, v.seq, target_cases)
...
def _identify_target_region(chrom, sequence, target_cases='ACGT'):
    logger = logging.getLogger(__name__)
